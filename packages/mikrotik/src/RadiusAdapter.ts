...
import { getFirestore, logger } from '@agentclaw/kernel';
...
export class RadiusAdapter {
  ...
  start(): void {
    ...
    this.server.on('message', async (msg, rinfo) => {
      try {
        ...
        logger.error(`RADIUS error: ${(err as Error).message}`);
      }
    });
    ...
    this.server.bind(this.port, () => {
      logger.info(`RADIUS server listening on port ${this.port}`);
    });
  }
  ...
  private async handlePacket(msg: Buffer, rinfo: dgram.RemoteInfo): Promise<void> {
  ...
  private async handleAuth(
    ...
    if (snap.empty) {
      logger.warn(`RADIUS AUTH REJECT: voucher not found "${username}"`);
    ...
    if (encryptedPass && encryptedPass.length > 0) {
      ...
      const md5 = crypto.createHash('md5').update(Buffer.concat([Buffer.from(this.secret), authenticator])).digest();
      ...
      if (passStr !== username) {
         logger.warn(`RADIUS AUTH REJECT: Invalid password for voucher "${username}"`);
         this.sendReject(id, authenticator, 'Invalid password', rinfo);
         return;
      }
    }

    logger.info(`RADIUS AUTH ACCEPT: ${username}`);
  ...
  private async handleAccounting(
    ...
    if (!sessionSnap.empty) {
      ...
      if (statusType === 2) {
        ...
        logger.info(`RADIUS ACCT STOP: ${username} (↑${totalOutput} ↓${totalInput})`);
      } else {
        // Start or interim
        await sessionDoc.ref.update({
          bytesUp: totalOutput.toString(),
        ...
        logger.debug(`RADIUS ACCT UPDATE: ${username} (↑${totalOutput} ↓${totalInput})`);
      }
    }
  ...
  private sendAccept(id: number, authenticator: Buffer, rinfo: dgram.RemoteInfo): void {
  ...
  private sendReject(id: number, authenticator: Buffer, msg: string, rinfo: dgram.RemoteInfo): void {
  ...
  private sendAccountingResponse(id: number, authenticator: Buffer, rinfo: dgram.RemoteInfo): void {
  ...
  private send(
    ...
    const md5 = crypto.createHash('md5').update(md5Input).digest();
    md5.copy(response, 4);
  ...
  private buildAttr(type: number, value: Buffer): Buffer {
  ...
  private parseAttributes(buf: Buffer): Map<number, Buffer> {
