...
class HotspotAgent {
    ...
    async start() {
        ...
        kernel_1.cronManager.schedule('hotspot-session-sync', '*/30 * * * * *', () => this.syncSessions());
        kernel_1.logger.info(`${this.name} started (hardware linkage enabled)`);
    }
    async stop() {
        kernel_1.cronManager.stop('hotspot-session-sync');
    }
    async syncSessions() {
        ...
        try {
            ...
            if (!snap.empty) {
                ...
                await batch.commit();
                kernel_1.logger.debug(`HotspotAgent: Synced ${snap.size} active sessions.`);
                kernel_1.eventBus.publish('hotspot:session-count', { count: snap.size }, this.name);
            }
        }
        catch (err) {
            kernel_1.logger.error(`HotspotAgent: Session sync failed: ${err.message}`);
        }
    }
    async deployHotspot(inputs) {
        const db = (0, kernel_1.getFirestore)();
    ...
    async loginUser(inputs) {
        ...
        if (!planSnap.exists)
            throw new Error('Plan not found for voucher');
        ...
        if (hotspot.routerIp && hotspot.routerIp !== 'local') {
            try {
                ...
                kernel_1.logger.info(`Hardware provisioned: ${voucher_code} on ${hotspot.routerIp}`);
            }
            catch (err) {
                kernel_1.logger.error(`Hardware provisioning failed for ${voucher_code}: ${err.message}`);
        ...
        kernel_1.eventBus.publish('session:started', { sessionId: sessionRef.id, macAddress: mac_address, hotspotId: hotspot_id }, this.name);
        return { sessionId: sessionRef.id, status: 'authenticated' };
    }
}
exports.HotspotAgent = HotspotAgent;
//# sourceMappingURL=HotspotAgent.js.map