...
async function verifyPromptComposition() {
    Logger_1.logger.info('--- OPENCLAW PROMPT COMPOSITION TEST START ---');
    try {
        ...
        if (prompt.includes('[SYSTEM IDENTITIES]') &&
            ...
            Logger_1.logger.info('SUCCESS: Prompt composer correctly assembled the components including IDENTITY.md.');
        }
        else {
        ...
        Logger_1.logger.error(`FAILURE: ${err.message}`);
        process.exit(1);
    }
    Logger_1.logger.info('--- OPENCLAW PROMPT COMPOSITION TEST END ---');
}
verifyPromptComposition();
//# sourceMappingURL=test-openclaw.js.map